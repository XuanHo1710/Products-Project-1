      ...
      div.innerHTML = `
        <div class="box-user">
          <div class="inner-avatar">
            <img src="/img/avatar.png" alt="${data.infoUserA.fullName}">
          </div>
...
socket.on("SERVER_RETURN_USER_ID_CANCEL_FRIEND", (data) => {
  ...
  // Trang danh sách người dùng
  const boxNotFriend = document.querySelector('[data-users-not-friend]');
  const idBox = boxNotFriend.getAttribute('data-users-not-friend');
  const div = document.createElement('div');
  div.classList.add('col-6');
  ...
  div.innerHTML = `
    <div class="box-user">
      <div class="inner-avatar">
        <img src="${data.infoUserA.avatar ? data.infoUserA.avatar : "/img/avatar.png"}"
      ...
      <div class="inner-info">
        ...
        <div class="inner-buttons">
          <button
            class="btn btn-sm btn-primary mr-1"
            btn-add-friend=${data.infoUserA._id}
          >
            Kết bạn
          </button>
          <button
            class="btn btn-sm btn-secondary mr-1"
            btn-cancel-friend="${data.infoUserA._id}"
          >
            Hủy
          </button>
        </div>
  ...
  `;
  if(idBox === data.userIdB){
    boxNotFriend.appendChild(div);
  }
  // Kết bạn
    const BtnAdd = div.querySelector('[btn-add-friend]');
    AddFriend(BtnAdd);
  // Hủy bạn
    const BtnCancel = div.querySelector('[btn-cancel-friend]');
    CancelFriend(BtnCancel);
});
